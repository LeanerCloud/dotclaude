...
7. **Prefer elegance to hacks** — On non-trivial changes, pause and ask "is there a cleaner way?" before shipping. Cleaner means simpler, not more elaborate. If a fix feels hacky, do it right. Skip for obvious one-liners. (§5)
...
- **Simplicity First (YAGNI)**: make every change as simple as possible. Build only what a current caller needs; no parameters, flags, hooks, or abstraction layers for a future that hasn't arrived. Sophistication is a cost, not a virtue. Detail and the pre-PR self-check in `coding-standards.md` ("Simplicity & Scope (YAGNI)").
...
- **Comment sparingly**: default to no comment; add one only where the *why* isn't deducible from the code, and keep it to 1-2 lines. Rationale belongs in the PR description, not the source. Detail in `coding-standards.md` ("Comments").
...
- **Background-first: don't block the main chat.** Default to running independent or long-running work in the background so the main session stays responsive instead of waiting on it: spawn subagents with `run_in_background: true` (builds, full test suites, CI/deploy/CR watchers, migrations, broad sweeps, research fan-outs, anything that takes more than ~30s or fans out), and use Bash `run_in_background: true` for long shell commands. Keep moving on other independent work or hand control back to the user while it runs, and collect results when the completion notification arrives - **never poll** (`TaskOutput`/status loops just re-block you). Run in the **foreground only** when the very next step truly needs that result, or for the 'When NOT to use subagents' carve-outs (tight debugging loops, interactive refinement). Fire independent calls together: multiple queries -> one message with parallel `Agent` calls. Lock patterns for concurrent agents in `multi-agent-comms.md`. **Reap background agents at their terminal condition**: a long-lived watcher (`ci-watch`/`cr-watch`/`merge-watch`) that emits an `idle_notification` is "waiting for more," not "done," so it stays live until the session exits unless it returns a terminal final message or is `TaskStop`ped. Write watcher prompts to END when their job is done, and `TaskStop` any still parked once their PR reaches a terminal state; a session is not clean to close with watchers still "running" for already-merged/closed PRs (full rule: `git-workflow.md` "Watcher teardown invariant").
...
- **Workflow improvements - self-update via PR**: when, while following these instructions, you notice a gap in the `~/.claude` workflow guidance (`CLAUDE.md` or any linked file) - missing guidance, ambiguous or contradictory wording, outdated advice, or a gap that just caused friction or a mistake - don't just mention it in chat: **capture it as a pull request against the dotclaude repo** (`LeanerCloud/dotclaude`). Mechanics: first **open a GitHub issue** on the dotclaude repo describing the gap (symptom, where the guidance falls short, and the proposed direction; triage-label it if the repo uses the rubric); branch off `origin/main` (`chore/<slug>` or `docs/<slug>`), make the minimal focused edit, commit (conventional commit, no Anthropic/Claude mentions per `git-workflow.md`), push, and `gh pr create` whose body explains the fix and **links the issue with `Closes #<n>`** so the two stay connected and the issue auto-closes on merge; batch several gaps noticed in the same session into one issue + PR pair. **Guardrails**: the PR is the approval gate - NEVER push straight to `main` or self-merge the dotclaude change; one coherent concern per PR; raise a PR only for genuine, reusable guidance gaps (skip noise and one-off preferences); after pushing, restore whatever branch was originally checked out so the working tree is left as found; and surface the PR link to the user. For a project-level `CLAUDE.md`, open the PR against that project's own repo instead. This rule supersedes the older "propose and wait" behavior: opening the PR *is* the proposal, and merging it stays the user's call.
...
- For non-trivial changes: pause and ask "is there a simpler way?" **Elegance means fewer moving parts, not more sophisticated ones**: fewer lines, fewer concepts, fewer names. If the "more elegant" version is longer or adds a concept, it isn't more elegant, it's over-engineering. Read this bullet as a prompt to *remove* machinery, never as an invitation to add it; self-check in `coding-standards.md` ("Simplicity & Scope (YAGNI)").
