...
- **Routine PR-shipping defaults to Sonnet end-to-end.** The standard pattern of plan + 3-pass review + worktree + implement + test + push + open-PR + ping-CR + arm-CI-watcher is Sonnet-tier work even *including* the plan and the review iterations. Empirically: when the issue's acceptance criteria are clearly enumerated, Sonnet's 3-pass review catches the right things and Sonnet's implementation passes the verification suite cleanly. Only escalate to Opus per-fork when a specific signal during the review surfaces "this needs real design judgment" (e.g., the shape isn't pre-decided, multiple competing approaches need weighing, the issue surfaces a subtle invariant that no acceptance criterion captured). For batched-PR-shipping work — every fork is Sonnet unless you can name the specific judgment call that warrants Opus.
