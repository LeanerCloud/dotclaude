...
- **No compound `cd && ...`**: shells with `cd` followed by other commands often require approval (e.g. `cd && git ...` triggers bare-repository-attack prevention). When you do need to operate from a different directory, use absolute paths in the command itself rather than chaining `cd` — e.g. `git -C /abs/path status` instead of `cd /abs/path && git status`, `npm --prefix /abs/path test` instead of `cd /abs/path && npm test`. Most CLIs (`git`, `npm`, `make`, `terraform`, `cargo`, `go`) accept a directory flag that removes the need for `cd` entirely.
