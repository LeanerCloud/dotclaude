...
- NEVER edit a runtime-injected attribution footer (`Generated with Claude Code` /
  `claude.ai/code/session...`) out of a PR body you touch. The runtime adds it on
  purpose, so leaving it in place is the rule. The no-Claude-mention rule covers
  commit messages.
