...
4. **Coordinate via the multi-agent-comms bus** (see the `multi-agent-comms` skill) — within a single session, disjoint chunks won't race on label edits. The race to worry about is **another concurrent session** triaging the same repo (or a project bot rewriting labels mid-pass). Acquire the appropriate `triage-<repo>` lock when starting; release when done.
