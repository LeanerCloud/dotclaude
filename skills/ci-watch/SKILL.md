...
3. Coordinate with sibling watchers before pushing a fix (message them, per the `multi-agent-comms` skill): another watcher may already be fixing a related failure, and two parallel pushes can stomp on each other or trigger a fresh round of CI for both. Run the push under the per-repo `git-push` lock from that skill.
