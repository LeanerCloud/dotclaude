...
description: The filesystem message bus and advisory lock protocol (`git-commit`, `git-push`,
  `test-runner`) that peer agent sessions use to share one repo without clobbering each other.
  Invoke when several agents or sessions work the same project concurrently.
---
